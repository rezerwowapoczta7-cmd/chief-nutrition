...
# --- TWOJE DANE ---
API_KEY = "AIzaSyAIGAClJpW7LbvpNE6XBJyHSJ3NvMJcXq4"
...
# --- RADAR MODELI ---
# Ta funkcja na żywo pobiera od Google listę działających modeli
...
def analyze_image_direct(image, note):
    ...
    # Pobieramy żywą listę modeli
    models_to_try = get_available_models()
    
    # Jeśli API nie odda listy, mamy plan B
    if not models_to_try:
        models_to_try = ["gemini-3-flash", "gemini-2.0-flash", "gemini-flash-latest"]
    ...
    for model in models_to_try:
        ...
        if response.status_code == 200:
            ...
            errors.append(f"{model} (Błąd {response.status_code})")
            continue
            
    raise Exception(f"Wyczerpano limity lub brak modeli. Logi: {errors}")
...
with tab1:
    ...
    if st.session_state.ai_result:
        ...
        if st.button("ZAPISZ DO DZIENNIKA"):
            C.execute("INSERT INTO posilki VALUES (?,?,?,?,?,?,?,?)", 
                      (dzis, data.get('nazwa', 'Danie'), data.get('kcal', 0), data.get('b', 0), data.get('t', 0), data.get('w', 0), 'AI', alert))
            CONN.commit()
            st.session_state.ai_result = None
            st.rerun()
